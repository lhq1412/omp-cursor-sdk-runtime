...
- Reject cloud `bc-*` agent IDs. Tools run only through OMP grants. Do not broaden explicit or empty grants. Native Cursor executors stay disallowed except hooked `read`/`grep`/`shell`/`edit`, and `webSearch` when OMP granted `web_search`.
