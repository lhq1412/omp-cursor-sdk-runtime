...
- Reuse requires the `native-checkpoint-v1` fingerprint of original raw joined system text + full messages; old flattened-history bindings rebootstrap. Never rebuild solely because of the incremental-send count; retain persisted `incrementalSendCount` compatibility. Context divergence, unsafe bindings, identity/grant changes, and failed-turn recovery retain their existing invalidation rules.
