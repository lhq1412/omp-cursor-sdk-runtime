...
- `src/session-runtime.ts` prepares/reuses agents, commits successful turns, and holds one SDK executor lease per session scope (`warmLocalExecutor` → `prewarmLocalExecutor`, released on scope dispose) so agent disposal never cold-restarts the local executor. `src/context.ts` selects budgeted native history separately from complete system + current-turn send text; divergent context (including raw system text) triggers fresh import. `src/native-history.ts` converts selected history into checkpoint blobs and reads settled `tokenDetails` occupancy through the public store; `src/sdk-session.ts` installs them and creates/resumes official local SDK agents. Empty history uses normal create; committed incremental and live parked-call continuation flows remain intact. Successful settles expose occupancy in `cursorSdk.contextOccupancy`, never `usage.contextTokens`; billing stays in `usage.orchestration`.
...
- Every granted OMP tool, including `web_search`, is exposed through the same SDK custom-tool bridge. `src/sdk-session.ts` allows only `mcp` when custom tools exist and no executors otherwise; Cursor-native business tools stay disabled. Do not reintroduce native-tool mapping or a separate search sidecar.
...
- Runtime Maps are keyed by session scope and agent instance. `src/session-resume.ts` folds/persists `cursor-sdk-agent-resume` custom entries in OMP JSONL; `src/store.ts` scopes SDK storage. Only matching committed handles are reusable. Branch navigation, compaction, failed turns, cwd/credential changes, and granted tool name/description/schema changes invalidate reuse; parked continuation fails closed on those contract changes. Lifecycle hooks dispose stale runtime state.
...
- Reject cloud `bc-*` agent IDs. Tools run only through OMP grants. Do not broaden explicit or empty grants. Cursor-native business executors stay disabled; SDK custom tools execute through OMP.
...
- On a fresh bootstrap, import selected history natively, not as flattened send text. Prepend complete OMP system instructions when nonempty to the current-turn send as `System instructions from OMP:\n${systemPrompt}` (including the first user turn). Join `string | string[]` with newlines and trim only outer whitespace for the send prefix. Preserve internal URLs, tool policies, LSP/find and delegation guidance, and custom template content; never remove sections by heading markers. Incremental sends omit that prefix; the existing agent retains the bootstrap.
- Native history import bypasses the SDK's initial environment tool-namespace advertisement. Include shared `SDK_TOOL_CONTEXT` plus `\n\n` only when bootstrap history is nonempty, after the complete OMP prefix and before current/recovery text; count its full text in the existing budget. Inclusion depends on history, not tool grants: the public `custom-user-tools` guidance says “any tools granted” and never expands grants. Import the complete current effective OMP history (existing compaction summary plus retained tail when applicable), or reject with a `/compact` instruction before sending, replacing bindings, or disposing the previous agent. Never silently trim history, including prefixes before a required recovery interaction. First-user/no-history and incremental sends omit the cue. Keep the default SDK system prompt native and separate from the OMP prefix; never copy SDK environment/user rules or schemas or hardcode model-specific discovery tool names.
...
- Reuse requires the `native-checkpoint-v1` fingerprint with `formatVersion: 3` of original raw joined system text + full messages. Older v2 and unversioned legacy fingerprints rebootstrap so agents cannot retain cropped system instructions; do not migrate journals or clear stores. Never rebuild solely because of the incremental-send count; retain persisted `incrementalSendCount` compatibility. Context divergence, unsafe bindings, identity/grant changes, and failed-turn recovery retain their existing invalidation rules.
- Keep agent reuse eligibility separate from consumed-input identity. Read validated v2 prefix digests and unversioned legacy message hashes to recognize committed history, never to reuse an old agent. Equal message count and fully matching history require `continueOnly: true` even when the agent must rebootstrap: import all messages and send a continuation, not the committed final user/developer request again. With appended input, keep the committed prefix in native history and send the new current input. Unverified fingerprints cannot establish that input was consumed.
...
- `src/omp-tools.ts`, `src/tool-catalog.ts`, `src/tools.ts`: current OMP tool definitions, grant boundaries, and the shared SDK custom-tool bridge.
...
Prioritize observable contracts: bootstrap versus incremental sends, complete system instructions on bootstrap only, old fingerprint rebootstrap, committed resume eligibility, branch/cwd/credential invalidation, current dynamic tool contracts and grants, deduplication, parked-call continuation and cancellation, compact followed by recorded-result recovery without replay, write-time portable tool-call IDs, provider-neutral legacy session ID migration that does not collapse duplicate-corrupted JSONL, parked/host correlation on OMP IDs, SDK callback dedupe remaining on raw SDK IDs, `/cursor-fast` session fold, honest `/cursor-refresh-models` failures, `/cursor-usage` using agent totals rather than summing listed turns or pricing `usage.cost`, native `/usage` remaining credential-scoped live agents, and checkpoint occupancy staying out of Run billing. No coverage threshold is configured.

CI has two jobs: `verify` (offline: install, typecheck, unit/host compatibility tests, OMP contract probe, boundaries, package dry-run, packed-plugin install) then `sdk-probe` (live Cursor SDK; needs `verify`). `scripts/omp-contract-probe.ts` verifies native-history codec/model/tool contracts and exact OMP package coherence without Cursor network access. `scripts/sdk-contract-probe.ts` checks that native `AgentOptions.systemPrompt` stays omitted, imports completed tool history and trailing user/developer messages through the production importer, prefixes only its first imported-history send with the shared `SDK_TOOL_CONTEXT`, verifies a new custom-tool callback and its `toolCallId`, checks settled new-root occupancy, and checks history retention after `Agent.resume` without replaying completed tools. After those native checks pass, it spawns isolated `--cancellation-case cancel|dispose` children, records `CAPABILITY` JSON, and treats a child non-zero exit as probe exit 2 without rewriting PASS lines. A sole `customToolArgEvents` failure caused by SDK `Agent Looping` / repeating-response stops is retried once before failing the gate. CI requires a dedicated `CURSOR_API_KEY` Actions secret; missing credentials fail the release gate. Never upload local credentials without authorization or record keys in files/output.
