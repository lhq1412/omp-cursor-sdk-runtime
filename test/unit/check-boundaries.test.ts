...
test("deep OMP imports require an allowlisted owner and symbol", async () => {
	...
	try {
		...
		await Promise.all([
			...
			writeFile(join(root, "scripts/omp-contract-probe.ts"), 'import { fromBinary, toBinary } from "@oh-my-pi/pi-catalog/discovery/protobuf";'),
		]);
		...
		await Promise.all([
			...
			writeFile(join(root, "scripts/omp-contract-probe.ts"), 'import { fromBinary, unownedSymbol } from "@oh-my-pi/pi-catalog/discovery/protobuf";'),
		...
		expect(denied.stderr).toContain("scripts/omp-contract-probe.ts:1: @oh-my-pi/pi-catalog/discovery/protobuf");
