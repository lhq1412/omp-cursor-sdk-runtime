...
Bootstrap budgeting reserves the model's output limit, framing overhead, and 4096 tokens per current image, then removes oldest complete history interaction units without splitting tool calls/results. Necessary system instructions and the current input are never silently truncated: oversized **text** fails before opening/resuming an agent. Image payload size is not treated as model tokens and does not by itself produce a context-overflow error. The text estimator deliberately counts UTF-8 bytes conservatively rather than claiming exact tokenization. Unknown model limits fail explicitly. Backend overflow is reported separately from quota/rate limits; no automatic retries or tool reexecution are added.
