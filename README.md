...
On stock brew OMP, `context.tools` plus xd://-mounted tools (including enabled `mcp__*` MCP tools) are mapped to Cursor SDK custom tools. When the native hook installs, granted OMP `read`/`grep`/`bash`/`glob`/`write` are advertised as Cursor `read`/`grep`/`shell`/`glob`/`piWrite` (and `ls` when `read` is granted) and intercepted back to the OMP host (`ls` executes as OMP `read`; `piWriteArgs` execute as OMP `write`). Native `edit` is advertised only when both `read` and `write` are granted (`edit`/`writeArgs` execute as OMP `write`); `edit` alone stays custom. Cursor has no `write` tool name; `piWrite` is the proto full-file write. Native `shell` timeouts are Cursor milliseconds converted to OMP seconds. Native `grep`/`ls`/`piWrite` results reuse OMP's `buildPiFindResult`/`buildPiLsResult`/`buildPiWriteResult` codecs. `delete` and `task` stay disallowed. SDK `settingSources` is always `[]`; `mcpServers` is always `{}`.
...
followed by the current user/developer input (including a first turn), or an explicit continuation. For bootstrap with nonempty history, `SDK_TOOL_CONTEXT` plus a blank line sits after the sanitized system prefix and before the current input/continuation. Prior history is not flattened into this send. Incremental rounds omit both prefixes; first-user/no-history sends omit only the tool-context cue. Empty or missing system text omits the OMP system prefix. `activeUserInput` selects only the final user/developer message, never an older user request. The bounded reuse fingerprint hashes the original raw joined system text and the complete message prefix and requires `native-checkpoint-v1`: raw system changes and old flattened-history bindings force fresh import. Legacy `messageHashes[]` fingerprints remain readable but new resume entries persist one prefix digest instead of a hash per message.

Imported history bypasses the SDK's initial environment block advertising its public `custom-user-tools` namespace. The minimal cue tells the model to discover schemas there for remaining granted tools and that hooked native `read`/`grep`/`shell`/`edit`/`glob`/`write`/`ls` are available when the interceptor installed; it does not grant tools or copy SDK environment/user rules or schemas. Its full text counts toward the bootstrap budget. Nonempty history must fit in full together with the cue; it is never trimmed to omit the cue. The default SDK system prompt remains native and separate from the sanitized OMP prefix.
...
While `cursor-sdk` is active, the adapter cancels OMP **auto** compaction (`auto_compaction_start` → `session_before_compact` `{ cancel: true }`). Manual `/compact` remains entirely OMP-owned for every provider. After a settled Cursor native summary, the adapter decodes `ConversationSummaryArchive` coverage and, on the next main-session terminal settle, materializes it through `ctx.compact({ mode: "soft" })` as an OMP `CompactionEntry` with `details.kind === "cursor-sdk-native-summary"`. That entry is the cross-provider context rewrite; `/context` follows OMP compact accounting. The current SDK agent and checkpoint are kept; the disk resume handle is marked dirty so a process restart bootstraps from the compacted session. The next Cursor request lazy-rebases `sendState` onto the compacted prefix, or bootstraps if a foreign provider wrote turns in between. Own materialization does not bump the session owner generation. A completed summary still attaches `cursorSdk.summary.probe` with before/after checkpoint counts (hashes/lengths only, never archive text). Settled checkpoint occupancy stays on `cursorSdk.contextOccupancy` only — `usage.contextTokens` is never set. OMP stale-tool prune can still rewrite the journal and force bootstrap; that is not cancelled here.

Summary status, text, and count belong to the SDK run and survive parked-tool continuations. Each OMP assistant message receives a snapshot, so later summary updates do not rewrite earlier messages. Native summary staging uses the completed checkpoint observation, even when the summary event arrived before the final continuation; archive coverage and retained-history boundary validation remain required.

Checkpoint summary archives and their model messages are content-addressed blob references, resolved through the SDK's agent-scoped store. Successful settles also compare checkpoint summary generations against the run's baseline, so missing summary callbacks do not hide a completed summary. Native model-message counts and `windowTail` are not OMP turn counts: materialization requires an unambiguous, complete prefix of matching OMP interactions and a retained tail.

Missing blobs, unsupported content, incomplete interactions, or an OMP prefix omitted by an older native import prevent automatic materialization rather than discard uncovered history. Cursor's own context occupancy can therefore shrink while OMP `/context` remains high. When checkpoint decoding or coverage alignment cannot be verified, the next main-session terminal settle warns once per session/agent and recommends normal `/compact`; the completed turn stays successful. Existing session files are not repaired in place, and partial Cursor summaries never replace uncovered OMP history.

Local agents are bound to the current OMP JSONL session leaf, cwd, and credential identity. Same-session incremental turns reuse the agent and send only the current user/developer input, or an explicit continuation when no new input exists. Reuse has no incremental-send count limit. Parked tool callbacks retain precedence. Parked continuation fails closed if cwd, credentials, the webSearch grant, or the hooked native tool set change; a changed ordinary custom-tool grant set does not. Fresh agents import the complete current effective history as native checkpoint blobs, then send the separate sanitized system prefix, the tool-context cue for nonempty history, and current input/continuation. Empty history uses normal `Agent.create`. Resume records persist the agent's execution cwd; a matching committed handle can be resumed after process restart. Branch navigation, ordinary OMP compaction, failed turns, and cwd/key changes still force fresh bootstrap. Cursor-native summary materialization preserves the in-memory agent and lazy-rebases.
