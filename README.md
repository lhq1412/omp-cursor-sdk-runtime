...
Missing blobs, unsupported content, incomplete interactions, or an OMP prefix omitted during native import prevent automatic materialization rather than discard uncovered history. Cursor's own context occupancy can therefore shrink while OMP `/context` remains high. Manual `/compact` remains available in that case; existing session files are not repaired in place.

Local agents are bound to the current OMP JSONL session leaf, cwd, and credential identity. Same-session incremental turns reuse the agent and send only the current user/developer input, or an explicit continuation when no new input exists. Parked tool callbacks retain precedence. Parked continuation fails closed if cwd, credentials, the webSearch grant, or the hooked native tool set change; a changed ordinary custom-tool grant set does not. Fresh agents import budgeted prior history as native checkpoint blobs, then send the separate sanitized system prefix, the tool-context cue for nonempty history, and current input/continuation. Empty history uses normal `Agent.create`. Resume records persist the agent's execution cwd; a matching committed handle can be resumed after process restart. Branch navigation, ordinary OMP compaction, failed turns, cwd/key changes, and the existing 20-completed-incremental-send threshold start a fresh bootstrap. Cursor-native summary materialization is the exception: it preserves the in-memory agent and lazy-rebases.
