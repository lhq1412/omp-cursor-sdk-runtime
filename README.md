...
Agent reuse eligibility and consumed-input identity are separate. Validated v2 prefix digests and legacy message hashes still identify committed history during migration. When that history exactly matches the current messages, the new agent imports every message and receives only a continuation plus bootstrap instructions—not the already committed final user/developer request again. When new input is appended, only the new final input becomes the send text. Old fingerprints never authorize reuse of the old agent.
