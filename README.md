...
Cursor SDK models advertise `contextManagement.owner = "provider"`. While they are active, OMP automatic compaction, prune, shake, speculation, and promotion stay off. Cursor summaries live on `cursorSdk.summary` and never rewrite the OMP journal. Settled occupancy sets `usage.contextTokens` from the checkpoint and `usage.contextTokensScope = "provider"`, so another provider cannot treat that number as a transcript anchor. Switching back with foreign turns since the last Cursor checkpoint bootstraps a new agent. Manual `/compact` still rewrites OMP history and invalidates the SDK runtime.
