...
While `cursor-sdk` is active, the adapter cancels OMP **auto** compaction (`auto_compaction_start` → `session_before_compact` `{ cancel: true }`). Manual `/compact` still runs and invalidates the SDK runtime. Cursor summaries stay on `cursorSdk.summary` and are kept as a deferred portable candidate (process-local). They are not applied to the OMP journal until another provider actually needs compact: `session_before_compact` then returns OMP's `firstKeptEntryId` / `tokensBefore` with the Cursor summary text if that summary's fingerprint covers the dropped prefix (overlap allowed, gaps not). Settled checkpoint occupancy stays on `cursorSdk.contextOccupancy` only — `usage.contextTokens` is never set. Switching back with foreign turns since the last Cursor checkpoint bootstraps a new agent. OMP stale-tool prune can still rewrite the journal and force that bootstrap; that is not cancelled here.
