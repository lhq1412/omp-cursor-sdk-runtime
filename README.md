...
While `cursor-sdk` is active, the adapter cancels OMP **auto** compaction (`auto_compaction_start` → `session_before_compact` `{ cancel: true }`). Manual `/compact` still runs and invalidates the SDK runtime. Cursor summaries stay on `cursorSdk.summary`. Settled checkpoint occupancy stays on `cursorSdk.contextOccupancy` only — `usage.contextTokens` is never set, so OMP keeps journal-sized context accounting and other providers are not poisoned by a 47k native window. Switching back with foreign turns since the last Cursor checkpoint bootstraps a new agent. OMP stale-tool prune can still rewrite the journal and force that bootstrap; that is not cancelled here.
