...
On stock brew OMP, `context.tools` plus xd://-mounted tools (including enabled `mcp__*` MCP tools) are mapped to Cursor SDK custom tools. When the native hook installs, granted OMP `read`/`grep`/`bash`/`glob`/`write` are advertised as Cursor `read`/`grep`/`shell`/`glob`/`edit` (and `ls` when `read` is granted) and intercepted back to the OMP host (`edit`/`writeArgs` executes as OMP `write`; `ls` executes as OMP `read`). Cursor has no `write` tool name. `delete` and `task` stay disallowed. SDK `settingSources` is always `[]`; `mcpServers` is always `{}`.
...
While `cursor-sdk` is active, the adapter cancels OMP **auto** compaction (`auto_compaction_start` → `session_before_compact` `{ cancel: true }`). Manual `/compact` remains entirely OMP-owned for every provider and invalidates the SDK runtime. Cursor summaries stay on `cursorSdk.summary`, with explicit `running` / `completed` state; a new summary cycle clears the previous text. They never rewrite the OMP journal or become an OMP `CompactionResult` because the public SDK does not expose the exact summarized-through message boundary. Settled checkpoint occupancy stays on `cursorSdk.contextOccupancy` only — `usage.contextTokens` is never set. Switching back with foreign turns since the last Cursor checkpoint bootstraps a new agent. OMP stale-tool prune can still rewrite the journal and force that bootstrap; that is not cancelled here.
