...
If those callbacks are gone (for example after a restart), complete recorded text tool results can continue on a **fresh agent** instead. Recovery validates the latest assistant batch's call/result IDs, keeps the initiating request and completed interaction within the bootstrap budget, and sends an explicit continuation rather than resending the old request as new input. A saved agent handle is not resumed for this path. Missing, duplicate or unmatched results, missing initiating requests, non-text tool results, and recovery evidence that cannot fit the budget fail explicitly. Existing live callbacks still resume normally without model rediscovery.
...
Bootstrap budgeting reserves the model's output limit, framing overhead, and current images, then removes oldest complete history interaction units without splitting tool calls/results. Necessary system instructions and the current input are never silently truncated: oversized input fails before opening/resuming an agent. The estimator deliberately counts UTF-8 bytes conservatively rather than claiming exact tokenization; images reserve at least 4096 tokens or their encoded byte size, whichever is larger. Unknown model limits fail explicitly. Backend overflow is reported separately from quota/rate limits; no automatic retries or tool reexecution are added.

## Results, usage, and diagnostics

Final SDK results fill missing answer text against the current answer step, without replaying tools or duplicating an already streamed answer. Public `run.usage` / `RunResult.usage` counters map input, output, cache reads/writes and reasoning tokens into OMP usage. A high-water mark belongs to the SDK Run, so parked returns report only newly observed usage, not the same cumulative counts again.

Assistant messages carry `cursorSdk` availability metadata: `tokenUsage` is `actual` or `unavailable`; `contextOccupancy` is a latest-turn estimate or unavailable, never the cumulative Run total. Authoritative OMP `contextTokens` stays unset. `cost` is currently `unavailable`: OMP requires numeric cost fields, so their zero placeholders **do not mean free usage**. The adapter does not guess billing attribution from agent-wide records; local billed IDs are per-turn identities, not SDK Run IDs, and costs can settle later. See the [public SDK usage contracts](https://cursor.com/docs/sdk/typescript#token-usage).
